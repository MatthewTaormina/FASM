...
pub async fn register_route(
    ...
    let manifest = match state.registry.get_app(&ns, &app).await {
        Some(m) => m,
        None    => return (StatusCode::NOT_FOUND, format!("app '{}/{}' not found", ns, app)).into_response(),
    };
    ...
    if let Err(e) = state.registry.add_route_record(&ns, &app, record.clone()).await {
...
pub async fn unregister_route(
    ...
    let route_id = match Uuid::parse_str(&route_id_str) {
        Ok(id)  => id,
        Err(_)  => return (StatusCode::BAD_REQUEST, "invalid route_id (expected UUID)").into_response(),
    };
    ...
    if let Err(e) = state.registry.remove_route_record(&ns, &app, route_id).await {
