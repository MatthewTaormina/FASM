...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/v1/namespaces/:ns",
            get(namespace::get_namespace)
            .delete(namespace::delete_namespace))

        // ── Apps ────────────────────────────────────────────────────────────
        .route("/api/v1/namespaces/:ns/apps",
            get(apps::list_apps)
            .post(apps::create_app))
        .route("/api/v1/namespaces/:ns/apps/:app",
            get(apps::get_app)
            .delete(apps::delete_app))

        // ── Files ───────────────────────────────────────────────────────────
        .route("/api/v1/namespaces/:ns/apps/:app/files",
            get(files::list_files))
        .route("/api/v1/namespaces/:ns/apps/:app/files/:filename",
            put(files::upload_file)
            .get(files::download_file)
            .delete(files::delete_file))

        // ── Routes (entry points) ────────────────────────────────────────────
        .route("/api/v1/namespaces/:ns/apps/:app/routes",
            get(routes::list_routes)
            .post(routes::register_route))
        .route("/api/v1/namespaces/:ns/apps/:app/routes/:route_id",
